...
from common import (
    ...
    CT, CC, CZ, CD, CN, CS, CL, CSP, CG, CG_MINOR, CG_MICRO, CV, CM, CR,
...
from .truss_math import analyze, compute_diagrams, compute_node_force_vectors, udl_local_components
...
class TrussApp:
    ...
    def _build_ui(self):
        ...
        tk.Radiobutton(diag_hdr, text='Truss diagrams', variable=self.diagram_mode,
                       value='truss', bg='#f5f5f3', font=('Helvetica',9),
                       command=self._draw_diagrams_only).pack(side='left', padx=(14,2))
        tk.Radiobutton(diag_hdr, text='Vierendeel diagram', variable=self.diagram_mode,
                       value='vierendeel', bg='#f5f5f3', font=('Helvetica',9),
                       command=self._draw_diagrams_only).pack(side='left', padx=2)
    ...
    def _remove_load(self):
        ...
        self.results=None; self.load_frame.pack_forget(); self._draw()
    ...
    def _remove_support(self):
        ...
        self.results=None; self.sup_frame.pack_forget(); self._draw()
    ...
    def _draw(self):
        ...
        if show_def:
            ...
            for rod in self.rods:
                ...
                na,nb = self.nodes[rod['a']],self.nodes[rod['b']]
                ax=na[0]+ra['ux']/1000*PX_PER_M*def_scale
                ay=na[1]+ra['uy']/1000*PX_PER_M*def_scale
                bx=nb[0]+rb['ux']/1000*PX_PER_M*def_scale
                by=nb[1]+rb['uy']/1000*PX_PER_M*def_scale
                asx,asy=w2s(ax,ay); bsx,bsy=w2s(bx,by)
                c.create_line(asx,asy,bsx,bsy,fill=CD,width=2.5,dash=(6,4))
    ...
    def _draw_vierendeel_diagram(self):
        ...
        def make_T(origin_x):
            def T(wx, wy):
                return (origin_x + pad + (wx-xmin)*scale_geo,
        ...
        amp_V = max_offset / max_V
        amp_M = max_offset / max_M
    ...
    def _show_node_vectors_report(self):
        ...
        for ni in range(len(self.nodes)):
            ...
            self._draw_node_fbd(cv, vecs, ld, rxn, size=FBD_SIZE)

            # -- numeric listing -------------------------------------------------
            info = tk.Text(row, width=34, height=max(6, len(vecs)*2+3),
            ...
            if rxn:
                info.insert('end',
                    f'Rxn({rxn["type"]}) Fx={rxn.get("rx",0):+7.2f}  '
                    f'Fy={-rxn.get("ry",0):+7.2f} kN\n')
